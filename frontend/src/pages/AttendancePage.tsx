...
const AttendancePage = () => {
  ...
  return <div className="text-white max-w-6xl mx-auto pb-10">
    ...
    <section className="rounded-3xl overflow-hidden border border-[#3d2e24] bg-gradient-to-l from-[#2b1916] via-[#1a1410] to-[#201813] p-6 md:p-9 mb-6 relative">
      <div className="absolute w-64 h-64 rounded-full bg-[#9c6b4a]/10 -left-20 -top-24" />
    ...
    <section className="grid lg:grid-cols-[1.1fr_.9fr] gap-6 mb-6"><article className="bg-[#14141f] border border-[#2c2525] rounded-3xl p-5 md:p-6"><div className="flex items-start justify-between mb-5"><div><p className="text-[#e8c39e] text-xs tracking-wider">MY SHIFT</p><h2 className="text-xl font-bold mt-1">سجّل دوامك</h2></div><span className="text-2xl">🕒</span></div><div className="grid sm:grid-cols-3 gap-2 mb-5">{shifts.map((item) => <button key={item.id} onClick={() => setShift(item.id)} className={`text-right rounded-2xl border p-4 transition ${shift === item.id ? "bg-[#9c6b4a] border-[#9c6b4a] shadow-lg shadow-[#9c6b4a]/20" : "border-[#3d2e24] hover:border-[#8b5242] bg-[#1a1410]"}`}><span className="text-xl">{item.icon}</span><strong className="block text-sm mt-2">{item.label}</strong><small className={shift === item.id ? "text-white/75" : "text-[#806f62]"}>{item.time}</small></button>)}</div><div className="rounded-2xl bg-[#1a1410] p-4 flex flex-wrap gap-3 justify-between items-center"><div><p className="font-semibold text-sm">{shiftLabel[shift]}</p><p className="text-xs text-[#a89080]">{currentRecord ? `بدأت في ${time(currentRecord.checkIn)}` : "لم تسجل حضورك في هذا الشيفت"}</p></div>{currentRecord ? <button disabled={pending} onClick={() => action("out")} className="px-5 py-3 rounded-xl bg-white text-[#1a1410] font-bold text-sm disabled:opacity-50">{pending ? "..." : "تسجيل الانصراف"}</button> : <button disabled={pending} onClick={() => action("in")} className="px-5 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 font-bold text-sm disabled:opacity-50">{pending ? "..." : "تسجيل الحضور"}</button>}</div></article>
      <article className="bg-[#14141f] border border-[#2c2525] rounded-3xl p-5 md:p-6"><p className="text-[#e8c39e] text-xs tracking-wider">LIVE TEAM</p><h2 className="text-xl font-bold mt-1 mb-5">الموجودون الآن <span className="text-[#e8c39e]">({data.onShift?.length || 0})</span></h2><div className="space-y-3 max-h-72 overflow-auto pr-1">{data.onShift?.length ? data.onShift.map((record: any) => <div key={record._id} className="flex items-center gap-3 bg-[#1a1410] rounded-2xl p-3"><div className="w-10 h-10 rounded-full bg-[#9c6b4a]/15 text-[#f69baa] grid place-items-center font-bold">{record.user?.name?.slice(0, 1) || "؟"}</div><div className="flex-1"><strong className="block text-sm">{record.user?.name || "—"}</strong><small className="text-[#a89080]">{record.user?.role || "موظف"} · {shiftLabel[record.shift]}</small></div><span className="text-xs text-emerald-300">● حاضر</span></div>) : <div className="text-center py-12 text-[#6f6258] text-sm">لا يوجد موظفون في الشيفت حالياً</div>}</div></article></section>
